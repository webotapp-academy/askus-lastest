...
$required = ['owner_name', 'email', 'phone', 'password', 'store_name', 'address', 'city', 'state', 'pincode'];
...
try {
    ...
    $stmt = $pdo->prepare("INSERT INTO vendors 
        (uuid, user_id, owner_name, email, phone, password, store_name, store_slug, address, city, state, pincode, gst_number, pan_number, store_description, status, rating, total_ratings, created_at, updated_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 0.00, 0, NOW(), NOW())");
