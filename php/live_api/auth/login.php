...
try {
    ...
    // Helper: try fetching a record by phone first (if provided or if not an email),
    // otherwise try by email. This prefers phone-based login while preserving email.
    ...
    // If not a regular user, attempt vendor login (same phone-preferred logic)
    $vendor = null;
    if (!empty($input['phone'])) {
    ...
    if ($vendor['status'] !== 'approved') {
        jsonResponse(['success' => false, 'message' => 'Vendor account is pending approval'], 403);
    }

    // Fetch full vendor details + subscription plan (same as profile.php)
    $vStmt = $pdo->prepare("
        SELECT
            v.*,
            sp.max_listings,
