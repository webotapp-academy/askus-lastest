...
try {
    ...
    $mrp = floatval($input['mrp'] ?? $input['price'] ?? 0);
    $sellingPrice = floatval($input['selling_price'] ?? $input['compare_price'] ?? $mrp);
    
    $stockQuantity = intval($input['stock'] ?? $input['stock_quantity'] ?? 0);
    $sku = trim($input['sku'] ?? '');
    $unit = trim($input['unit'] ?? 'piece');
    
    if (empty($name) || empty($description) || !$categoryId || $mrp <= 0) {
    ...
    if (!empty($_FILES['images'])) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($file['error'] === 0) {
                $imageUrl = uploadFile($file, 'products');
                if ($imageUrl) {
                    $isPrimary = ($i === 0) ? 1 : 0;
                    $pdo->prepare("INSERT INTO product_images (product_id, image_url, alt_text, sort_order, is_primary) VALUES (?, ?, ?, ?, ?)")
                        ->execute([$productId, $imageUrl, $name, $i, $isPrimary]);
                }
            }
        }
    }
